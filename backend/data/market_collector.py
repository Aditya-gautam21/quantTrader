    ...
    def download_historical_data(self, tickers, start_date, end_date, interval="1d"):
        ...
        try:
            ...
            if len(tickers) == 1 and isinstance(data.columns, pd.Index) and not isinstance(data.columns, pd.MultiIndex):
                data.columns = pd.MultiIndex.from_tuples([(col, tickers[0]) for col in data.columns])
...
if __name__ == "__main__":
    ...
    if historical is not None:
        ...
        aapl_close = historical[('Close', 'AAPL')]
        print(f"\nPrice Range for AAPL: ${aapl_close.min():.2f} - ${aapl_close.max():.2f}")
